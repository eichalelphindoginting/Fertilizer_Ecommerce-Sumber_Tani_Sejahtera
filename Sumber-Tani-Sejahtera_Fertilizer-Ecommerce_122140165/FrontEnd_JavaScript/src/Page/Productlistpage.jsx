...
const ProductList = () => {
  ...
  const fetchProducts = async () => {
    try {
      ...
      if (
        ...
        setError(
          response.data?.message ||
            "Gagal mengambil data produk dari API dengan format yang benar."
        );
      }
    } catch (err) {
      ...
      if (err.response) {
        ...
        errorMessage =
          "Tidak ada respons dari server. Periksa koneksi Anda atau pastikan server berjalan.";
      } else {
  ...
  return (
    <div className="min-h-screen bg-green-50 py-12 md:py-16 lg:py-20">
      {" "}
      {/* Adjusted padding */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-16">
        {" "}
        {/* pt-16 untuk spacer dari navbar fixed */}
        <div className="text-center mb-12 md:mb-16">
          <h1 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold text-green-700">
            {keyword
              ? `Hasil Pencarian untuk "${keyword}"`
              : "Semua Produk Pupuk Kami"}
          </h1>
          {!keyword && (
            <p className="mt-3 text-lg text-gray-600 max-w-2xl mx-auto">
              Temukan pupuk berkualitas tinggi untuk segala kebutuhan pertanian
              Anda.
            </p>
          )}
        </div>
